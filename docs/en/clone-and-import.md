...
`git clone https://github.com/ByteLegendQuest/java-fix-overload-method-selection`

A project directory named `java-fix-overload-method-selection` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-overload-method-selection` directory which is created just now.
